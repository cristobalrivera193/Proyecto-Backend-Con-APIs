...
class InsumoAdmin(admin.ModelAdmin):
    ...
    list_filter = ['nombre', 'tipo', 'color']
    prepopulated_fields = {"slug": ["nombre"]}

@admin.register(Pedido)
class PedidoAdmin(admin.ModelAdmin):
    list_display = ['id', 'origen_pedido', 'estado_pago', 'estado_pedido', 'email', 'vista_imagen']

    def vista_imagen(self, obj):
        if obj.imagen_Referencia:
            return format_html('<img src="{}" width="60" style="border-radius:6px;" />', obj.imagen_Referencia.url)
        return "Sin imagen"
